...
if [ "`echo $* | grep 64`" ]; then
 ...
 if [ "`echo $* | grep download`" ]; then
  ...
  PACKAGES='qtbase qscintilla2 mpfr eigen opencsg cgal glib freetype fontconfig harfbuzz libxml2 vtk6'
 fi
else
 ...
 if [ "`echo $* | grep download`" ]; then
  ...
  PACKAGES='qtbase qscintilla2 mpfr eigen opencsg cgal nsis glib freetype fontconfig harfbuzz libxml2 vtk6'
 fi
fi
